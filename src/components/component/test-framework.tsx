...
export function TestFramework({headline, getOptions, asChord}: TestFrameworkProps) {

    const [selected, setSelected] = useState<TestOption>()
    const [isCorrect, setIsCorrect] = useState<boolean | undefined>()
    const [options, setOptions] = useState(getOptions());

    const [correctOption, setCorrectOption] = useState<TestOption>(sample(options) as TestOption);
    ...
    return (
        ...
        <main className="w-full max-w-2xl mx-auto flex flex-col items-center gap-6 py-8 px-4">
            <div className="w-full flex flex-col items-center gap-4">
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{headline}</h2>
            ...
            <div className="w-full grid grid-cols-2 gap-4">
                {options.map(opt => {
                    ...
                    return <Button key={opt.text} className="h-16"
                                   variant={variant}
                                   onClick={async () => {
